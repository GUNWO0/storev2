package com.metacoding.storev2.store;


import jakarta.servlet.http.HttpServletRequest;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;

import java.util.List;

@RequiredArgsConstructor // 의존성 주입
@Controller
public class StoreController {
    private final StoreService storeService;

    // 상품 목록
    @GetMapping("/")
    public String list(HttpServletRequest request) {
        List<Store> storeList = storeService.상품목록();
        request.setAttribute("models", storeList);
        return "store/list";
    }

    // 상품 등록
    @GetMapping("/store/save-form")
    public String saveForm() {
        return "store/save-form";
    }

    @PostMapping("/store/save")
    public String save(StoreRequest.ProductDTO ProductDTO) {
        storeService.상품등록(ProductDTO);
        return "redirect:/";
    }

    // 상세보기
    @GetMapping("/store/{id}")
    public String detail(@PathVariable("id") int id, HttpServletRequest request) {
        Store store = storeService.상세보기(id);
        request.setAttribute("model", store);
        return "store/detail";
    }

    // 수정 된 상세보기
    @GetMapping("/store/{id}/update-form")
    public String updateForm(@PathVariable("id") int id, HttpServletRequest request) {

        request.setAttribute("model", storeService.상세보기(id));
        return "store/update-form";
    }

    // 상품 삭제
    @PostMapping("/store/{id}/delete")
    public String delete(@PathVariable("id") int id) {
        storeService.상품삭제(id);
        return "redirect:/";
    }

    // 상품 수정
    @PostMapping("/store/{id}/update")
    public String update(@PathVariable("id") int id, StoreRequest.ProductDTO ProductDTO) {
        storeService.상품수정(id, ProductDTO);
        return "redirect:/store/" + id;
    }

}
